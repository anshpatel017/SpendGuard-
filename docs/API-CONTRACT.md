...
> - **`GET /evaluation`** serves a detector's metrics only for the anomaly types it emits. The report scores D1 on split purchases (F1 0.000), which is true and meaningless, and the first dashboard build misread it. An `AblationRow`'s `configuration` is built from that arm's own run config, and its `triage_accuracy` comes from the arm's own report; `note_quality_score` stays null until the blinded rubric is graded. Ablation notes never enter `agent_metrics`.
